...
void OLED_Clear_rect(u8 x1,u8 y1,u8 x2,u8 y2)
{
	for(int i = x1;i<=x2;i++)
	{
		for(int j=y1; j<=y2;j++)
...
void OLED_ClearChar(u8 x,u8 y,u8 chr,u8 size1,u8 mode)
{
	u8 i,m,temp,size2,chr1;
	u8 x0=x,y0=y;
	if(size1==8)size2=6;
...
void OLED_ClearString(u8 x,u8 y,u8 *chr,u8 size1,u8 mode)
{
	while((*chr>=' ')&&(*chr<='~'))//�ж��ǲ��ǷǷ��ַ�!
	{
		OLED_ClearChar(x,y,*chr,size1,mode);
		if(size1==8)x+=6;
		else x+=size1/2;
		chr++;
  }
